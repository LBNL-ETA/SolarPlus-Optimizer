...
class FlexstatDriver(XBOSProcess):
    def __init__(self, cfg):
        ...
        self._default_heating_setpoint_map = self.thermostat_config.get("default_heating_setpoint_map", {'thermostat_west': 70, 'thermostat_east': 67})
        ...
        for response in responses:
            ...
            if control_flag != None:
                if not control_flag and self.device_control_flag.get(device, False):
                    self.change_setpoints(device=device, variable_name='heating_setpoint', new_value=self._default_heating_setpoint_map[device])
                    self.change_setpoints(device=device, variable_name='cooling_setpoint', new_value=self._default_cooling_setpoint_map[device])
        ...
        for device in device_setpoint_dict:
            ...
            if control_flag:
                ...
                if abs(time_now - first_change_time) > self._default_time_threshold:
                    ...
                    heating_setpoint = self._default_heating_setpoint_map[device]
                    cooling_setpoint = self._default_cooling_setpoint_map[device]
                    print("CASE 1/2")
                elif time_now < first_change_time:
                    heating_setpoint = setpoint_dict[first_change_time].get('heating_setpoint', None)
                    cooling_setpoint = setpoint_dict[first_change_time].get('cooling_setpoint', None)
